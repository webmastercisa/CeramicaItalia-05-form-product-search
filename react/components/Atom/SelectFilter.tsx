...
export const SelectFilter = ({dataList, onSelectItem, itemSelect, disabled}:Props) => {
  ...
  return (
    <div ref={refOutside} className={handles.selectContentFilter}>
      <button className={handles.selectBtnFilter} onClick={() => setShow(!show)} disabled={disabled}>
        {disabled ? <div></div> :itemSelect?.text} <ImgArrowSelectDown />
      </button>
      <div
        className={applyModifiers(
          handles.selectContentListFilter,
          show ? 'show' : ''
        )}
      >
        <ul className={handles.selectListUlFilter}>
          {dataList.map((dataSelect)=>{
            return <li key={dataSelect.id} className={handles.selectListLiFilter} onClick={() => onSelect(dataSelect)}>
                        {dataSelect.text}
                    </li>
          })}
        </ul>
